...
import { centerClip, detectPitchAutocorrelation, extractCyclesAndAmplitudes } from '../src/core/algorithms/pitch.js';
...
test('centerClip should correctly clip values based on threshold', () => {
  ...
  for (let i = 0; i < output.length; i++) {
    assert.ok(Math.abs(output[i] - expected[i]) < 1e-5, `At index ${i}, expected ${expected[i]} but got ${output[i]}`);
  }
});
...
test('detectPitchAutocorrelation should detect the correct frequency of a sine wave', () => {
  ...
  assert.ok(Math.abs(result.frequency - targetFreq) < 1.0, `Detected frequency ${result.frequency} is not close to target ${targetFreq}`);
});
...
test('extractCyclesAndAmplitudes should correctly identify cycles and amplitudes in a synthetic pulse train', () => {
  ...
  for (let i = 0; i < amplitudes.length; i++) {
    assert.ok(Math.abs(amplitudes[i] - expectedAmps[i]) < 1e-5, `Amplitude mismatch at ${i}: expected ${expectedAmps[i]} got ${amplitudes[i]}`);
  }
});
